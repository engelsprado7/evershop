...
/** Fetch all categories with Include in Menu = Yes (same as Menu query) */
async function getMenuCategories(pool) {
...
export default {
  Query: {
    basicMenuWidget: async (_, { settings }, { pool }) => {
      ...
      if (!menus) {
        return { menus: [], isMain, className: settings?.className };
      }
      ...
      if (pages.length > 0) {
        const pageQuery = select('cms_page.uuid')
